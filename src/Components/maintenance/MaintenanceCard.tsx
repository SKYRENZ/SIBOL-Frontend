...
export const MaintenanceCard: React.FC<MaintenanceCardProps> = ({
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 shadow-sm hover:shadow-md transition p-5 relative">
      ...
      {mode === 'pending' && (
        <div className="absolute top-3 right-3">
          <span className={`inline-block px-2 py-1 text-xs font-medium rounded-full border ${getStatusColor(ticket.Status)}`}>
      ...
      {/* Title */}
      <div className={`mb-3 ${mode === 'request' || mode === 'pending' ? 'pr-20' : ''}`}>
