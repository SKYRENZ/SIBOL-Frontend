...
const WasteCollectionTab: React.FC<WasteCollectionTabProps> = ({ parentSearchTerm = '' }) => {
  ...
  }, [pendingSelectId, containers, selectContainer, is3DMode]);
  ...
  useEffect(() => {
    const targetBoundary = combinedBoundary || boundary176e;
    if (targetBoundary && mapRef.current) {
      try {
        const [minLng, minLat, maxLng, maxLat] = bbox(targetBoundary);
  ...
  }, [combinedBoundary, boundary176e, is3DMode]);
    ...
    <div className="fixed inset-0 top-0 left-0 w-full h-screen bg-white flex flex-col">
      ...
      <div className={`fixed left-4 top-1/2 -translate-y-1/2 z-[1000] space-y-3 transition-all duration-300 ${ui.sidebarOpen ? 'w-80 opacity-100' : 'w-0 opacity-0 pointer-events-none'}`}>
        ...
        <div className="relative rounded-xl shadow-lg p-0 border border-gray-200 bg-white h-auto max-h-96 overflow-hidden flex flex-col">
          <div className="px-3 py-2 bg-gradient-to-r from-[#2E523A] to-[#4a7c5d] text-white rounded-t-xl flex items-center justify-between gap-2">
            <div className="flex items-center gap-2 shrink-0">
              ...
              <span className="font-semibold text-sm whitespace-nowrap">Map Legends</span>
            </div>
            <div className="flex items-center gap-1.5 overflow-hidden">
              <button
                onClick={() => setIs3DMode(!is3DMode)}
                className="bg-white/10 hover:bg-white/20 text-white text-[10px] sm:text-xs px-2 py-1 rounded border border-white/20 transition-all font-medium whitespace-nowrap flex-1 text-center"
              >
                {is3DMode ? '2D Mode' : '3D Mode'}
              </button>
              <button
                onClick={() => modals.add ? closeModal('add') : openModal('add')}
                className="bg-white/20 hover:bg-white/30 text-white text-[10px] sm:text-xs px-2 py-1 rounded border border-white/30 transition-all font-medium whitespace-nowrap flex-1 text-center shadow-sm"
              >
                {modals.add ? 'Close' : '+ Add'}
              </button>
            </div>
          </div>
        ...
        {!mapboxToken && (
          <div className="absolute inset-0 z-50 flex items-center justify-center bg-gray-100/80 backdrop-blur-sm pointer-events-none">
            <div className="bg-white p-4 rounded-xl shadow-lg border-2 border-red-200">
              <p className="text-gray-800 font-bold mb-1">Mapbox Token Missing</p>
              <p className="text-gray-600 text-sm">Please set VITE_MAPBOX_TOKEN in your .env file.</p>
            </div>
          </div>
        )}
        <Map
            ...
            .map((item) => (
              ...
              >
                <div
                  ...
                  onClick={(e) => { e.stopPropagation(); selectContainer(item); }}
                >
                  <WasteIconSVG />
                </div>
              </Marker>
