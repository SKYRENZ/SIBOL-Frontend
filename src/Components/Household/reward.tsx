...
interface RewardTabProps {
  // ✅ make optional so other files won’t break immediately
  // (filters are no longer used in this tab)
  filters?: string[];
  onEditReward: (reward: Reward) => void;
}

const RewardTab: React.FC<RewardTabProps> = ({ onEditReward }) => {
  ...
  const handleArchive = async (id: number) => {
    ...
    try {
      ...
      refetch();
      showSnack("Reward archived.", "success"); // ✅ snackbar
    } catch (err: any) {
      console.error("Failed to archive reward:", err);
      showSnack(err?.message || "Failed to archive reward", "error"); // ✅ removed alert
    }
  };
  ...
  const handleRestore = async (id: number) => {
    try {
      ...
      refetch();
      showSnack("Reward restored.", "success"); // ✅ snackbar
    } catch (err: any) {
      console.error("Failed to restore reward:", err);
      showSnack(err?.message || "Failed to restore reward", "error"); // ✅ removed alert
    }
  };

  // ✅ Filters removed: we rely only on showArchived + API/hook behavior
  const filteredData = rewards;
                  ...
                  <div className="p-6">
                    ...
                    <div className="flex items-center justify-between mb-4 pb-4 border-b border-gray-100">
                      ...
                      <div className="text-right">
                        ...
                        <p className="text-2xl font-bold text-gray-900">{reward.Quantity}</p>
                      </div>
                    </div>
